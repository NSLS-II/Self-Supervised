...
if [ "${kpicker}" == "YES" ]; then 
        ...
        while true; do
                ...
                sleep 1 
                python Self-Supervised/kpicking_cpu.py --input_dir 'linked' --output_dir 'Kpicker/aligned' \
                                --pre_trained_model 'test_model.h5' --star_file  'CtfFind/micrographs_defocus_ctf.star' \
                                --threshold $picking_threshold  --threads $picking_threads  --particle_size  $box_size --coordinate_suffix '_kpicker' \
                                --bin_size $bin_size    # --testRun
                ...
                while true; do
                        ...
                        for class in `(echo "$list")`; do
                                awk -v class=$class -v column=$column '{ if (NF>2 && strtonum($column) == class) print}' Class2D/c2d_search_it025_data.star  >>  tmp.star 
                        done
                        #cat tmp.star |shuf -n 1000 >> particles_selected.star
                        cat tmp.star >> particles_selected.star

                        ### manual selection of 2D classes to override automatically selected. 
                        if [ "${check}" == "YES" ]; then
                                `which relion_display` --gui --i Class2D/c2d_search_it025_model.star --allow_save \
                                        --fn_parts  particles_selected.star \
                                        --fn_imgs Select/class_averages.star \
                                        --recenter  --regroup 5
                        fi 
